...
void ProcessTouchInput(ImGuiIO& io)
{
    static int fd = -1;
    ...
    while (read(fd, &ev, sizeof(ev)) > 0)
    {
        if (ev.type == EV_ABS)
        {
            switch (ev.code)
            {
                ...
                case ABS_MT_POSITION_X: // x and y are correct under here, cuz they were swapped. This is correct.
    ...
    if (active_slot >= 0)
    {
        ...
        float x = (float)(touch.slots[active_slot].x - TOUCH_X_MIN) /
                  (float)(TOUCH_X_MAX - TOUCH_X_MIN) * SCREEN_WIDTH;
        float y = SCREEN_HEIGHT - (float)(touch.slots[active_slot].y - TOUCH_Y_MIN) /
                                (float)(TOUCH_Y_MAX - TOUCH_Y_MIN) * SCREEN_HEIGHT;
        ...
        if (updated)
            printf("📍 Touch[%d] pos=(%d,%d)\n",
                   active_slot,
                   SCREEN_WIDTH - touch.slots[active_slot].x,//for flipping the x axis
                   touch.slots[active_slot].y
					);
    }
...
int main(int, char**)
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "STM32MP1 Touch Debug", NULL, NULL);
    if (!window)
    {
    ...
    // Disable GLFW mouse handling to keep touch input
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Update touchscreen input AFTER ImGui frame start
        ProcessTouchInput(io);
