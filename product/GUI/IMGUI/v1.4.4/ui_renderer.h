...
public:
    ...
    void RenderAttendanceScreen(ImDrawList* draw_list,
        ...
        // Header
        std::string header = "Attendance (Last 30 Days)";
        ...
        // User label
        std::string user_label = "User: " + (user_name.empty() ? std::string("(unknown)") : user_name);
        ...
        // Column headers
        draw_list->AddText(ImVec2(60, 110), IM_COL32(60, 60, 60, 255), "Date");
        ...
        if (dates.empty()) {
            draw_list->AddText(ImVec2(60, start_y), IM_COL32(120, 120, 120, 255), "No attendance in the last 30 days");
        } else {
            int count = (int)dates.size();
            ...
            if (count > max_rows) {
                draw_list->AddText(ImVec2(60, start_y + max_rows * row_h + 6.0f), IM_COL32(120, 120, 120, 255), "(more not shown)");
            }
        }
        ...
        const char* confirm_text = "CONFIRM";
