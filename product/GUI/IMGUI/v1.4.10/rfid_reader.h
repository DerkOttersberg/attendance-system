...
public:
    ...
    // Flush any buffered RFID data and reset parser state
    void Flush() {
        if (fd < 0) return;
    ...
    bool SendCommand(const std::string& command) {
        if (fd < 0) {
            if (!Open()) {
                fprintf(stderr, "- Cannot send command: device not open\n");
                return false;
            }
        }
    ...
    bool Send_RED_OFF_Command() {
        return SendCommand("red_off");
    }
    bool Send_GREEN_ON_Command() {
        return SendCommand("green_on");
    }
    bool Send_GREEN_OFF_Command() {
        return SendCommand("green_off");
    }
