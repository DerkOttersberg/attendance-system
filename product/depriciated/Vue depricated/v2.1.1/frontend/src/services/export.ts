...
export async function generateFloorplanPDF() {
  ...
  const grouped = legendItems.reduce<Record<string, FloorplanLegendItem[]>>((acc, item) => {
  ...
  const roomNames = Object.keys(grouped).sort((a, b) => a.localeCompare(b))
  const lineHeight = 5
  ...
  for (const room of roomNames) {
    ...
    doc.setFontSize(11)
    doc.setFont('helvetica', 'bold')
    doc.text(room, 14, imageY)
    imageY += lineHeight

    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')

    const rows = grouped[room].sort((a, b) => {
    ...
    for (const item of rows) {
      ...
      const rowText = `${item.desk} - ${item.user} - ${item.slot}`
      doc.text(rowText, 18, imageY)
      imageY += lineHeight
    }
