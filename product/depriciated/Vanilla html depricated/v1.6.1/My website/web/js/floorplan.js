(() => {
    ...
    function renderRoomsManager() {
            ...
            return `
                <div class="floorplan-room-row" data-room-row="${room.id}">
                    ...
                    <div class="floorplan-room-actions">
                        <button class="btn btn-secondary" data-room-remove="${room.id}">Remove</button>
                    </div>
                </div>
            `;
        }).join('');
        ...
        elements.roomsPanel.innerHTML = `
            ...
            <div class="floorplan-room-add">
                <input type="text" placeholder="New room name" value="${escapeHtml(roomNameDraft)}" id="roomNameInput" />
                <button class="btn btn-primary" id="addRoomBtn">Add</button>
            </div>
        `;
        ...
        elements.roomsPanel.querySelectorAll('[data-room-input]').forEach(input => {
            input.addEventListener('change', event => {
                ...
                if (!roomId) return;
                const newLabel = event.target.value.trim() || 'Room';
    ...
    function removeRoom(roomId) {
        ...
        if (!confirm(`Remove room "${room.label}"? Desks inside will move to another room.`)) return;
    ...
    function renderTable() {
        ...
        desks.sort((a, b) => {
            ...
            if (key === 'desk') return compareValues(a.name || '', b.name || '', dir);
            if (key === 'room') return compareValues(getRoomLabel(a.room), getRoomLabel(b.room), dir);
            if (key === 'morning') {
                const aName = getUserName(getDeskAssignments(a.id, overviewDay).morning);
                const bName = getUserName(getDeskAssignments(b.id, overviewDay).morning);
                return compareValues(aName, bName, dir);
            }
            if (key === 'afternoon') {
                const aName = getUserName(getDeskAssignments(a.id, overviewDay).afternoon);
                const bName = getUserName(getDeskAssignments(b.id, overviewDay).afternoon);
                return compareValues(aName, bName, dir);
            }
            if (key === 'status') {
                const aStatus = getDeskStatus(getDeskAssignments(a.id, overviewDay)).label;
                const bStatus = getDeskStatus(getDeskAssignments(b.id, overviewDay)).label;
                return compareValues(aStatus, bStatus, dir);
            }
        ...
        const rows = desks.map(desk => {
            ...
            const searchText = `${desk.name || ''} ${getUserName(assignments.morning)} ${getUserName(assignments.afternoon)}`.toLowerCase();
            if (overviewSearch && !searchText.includes(overviewSearch)) {
                return '';
            }
        ...
        const headerRow = overviewMode === 'free'
            ? `
                <tr>
                    <th class="sortable" data-table="floorplan" data-sort="desk">Desk</th>
                    <th class="sortable" data-table="floorplan" data-sort="room">Room</th>
                    <th>Free Slots</th>
                    <th class="sortable" data-table="floorplan" data-sort="morning">Morning</th>
                    <th class="sortable" data-table="floorplan" data-sort="afternoon">Afternoon</th>
                    <th class="sortable" data-table="floorplan" data-sort="status">Status</th>
                    <th></th>
                </tr>
            `
            : `
        ...
        elements.table.querySelectorAll('th[data-table="floorplan"][data-sort]').forEach(th => {
    ...
    function updateFloorplanSortIndicators() {
        if (!elements.table) return;
        const ths = elements.table.querySelectorAll('th[data-table="floorplan"][data-sort]');
        ths.forEach(th => {
            th.classList.remove('sort-asc', 'sort-desc');
            const key = th.getAttribute('data-sort');
            if (key === overviewSort.key) {
                th.classList.add(overviewSort.dir === 'asc' ? 'sort-asc' : 'sort-desc');
            }
        });
    }

    function getExportLegend(day) {
        const legend = [];
        const targetDay = DAYS.includes(day) ? day : state.day;

        state.desks.forEach(desk => {
            const assignments = getDeskAssignments(desk.id, targetDay);
            if (assignments.morning || assignments.afternoon) {
                const slots = [];
                if (assignments.morning) slots.push('Morning');
                if (assignments.afternoon) slots.push('Afternoon');

                const sameUser = assignments.morning && assignments.afternoon
                    && assignments.morning === assignments.afternoon;

                if (sameUser) {
                    legend.push({
                        user: getUserName(assignments.morning),
                        desk: desk.name || 'Desk',
                        room: getRoomLabel(desk.room),
                        slot: slots.join(' & ')
                    });
                } else {
                    if (assignments.morning) {
                        legend.push({
                            user: getUserName(assignments.morning),
                            desk: desk.name || 'Desk',
                            room: getRoomLabel(desk.room),
                    ...
                    if (assignments.afternoon) {
                        legend.push({
                            user: getUserName(assignments.afternoon),
                            desk: desk.name || 'Desk',
                            room: getRoomLabel(desk.room),
                            slot: 'Afternoon'
                        });
