...
const API = {
    ...
    async fetchPointsList() {
        const response = await fetch(`${CONFIG.API_BASE}/api/points`);
        if (!response.ok) throw new Error('Failed to fetch points');
        return await response.json();
    },

    async setUserPoints(userId, points) {
        const response = await fetch(`${CONFIG.API_BASE}/api/points/${userId}`, {
            method: 'PUT',
        ...
        if (!response.ok) {
            ...
            throw new Error(text || 'Failed to update points');
        }
    ...
    async fetchUserPoints(userId) {
        const response = await fetch(`${CONFIG.API_BASE}/api/user-points/id/${userId}`);
        if (!response.ok) throw new Error('Failed to fetch user points');
        return await response.json();
    },

    async fetchFloorplan() {
        const response = await fetch(`${CONFIG.API_BASE}/api/floorplan`);
    ...
    async saveFloorplan(data) {
        ...
        if (!response.ok) {
            const text = await response.text();
            throw new Error(text || 'Failed to save floorplan');
        }
