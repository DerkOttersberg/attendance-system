...
window.autoRefreshEnabled = false; // default: disabled (clean refresh by default)
    ...
    if (enabled) {
        if (!autoRefreshIntervalId) {
            autoRefreshIntervalId = setInterval(() => {
                // perform a clean refresh that preserves selection/scroll
                loadAllData();
...
function toggleAutoRefresh() {
    setAutoRefresh(!window.autoRefreshEnabled);
}

document.addEventListener('DOMContentLoaded', () => {
    loadAllData();
    // default is off; if you want it on by default, call setAutoRefresh(true) here
});
