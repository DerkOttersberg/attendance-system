...
import type { AttendanceRecord, DepartmentRecord, PointsRecord, ProductRecord, UserRecord } from './types'

type AttendanceSortKey = 'name' | 'department' | 'clock_in' | 'clock_out' | 'duration' | 'date'
type UserSortKey = 'id' | 'name' | 'department'
type PointsSortKey = 'id' | 'name' | 'points'

type SortState = {
  today: { key: AttendanceSortKey; dir: 'asc' | 'desc' }
  all: { key: AttendanceSortKey; dir: 'asc' | 'desc' }
  users: { key: UserSortKey; dir: 'asc' | 'desc' }
  points: { key: PointsSortKey; dir: 'asc' | 'desc' }
}

const activeTab = ref<'today' | 'all' | 'users' | 'floorplan' | 'points'>('today')
...
const state = reactive({
  ...
  sort: {
    ...
    points: { key: 'name' as PointsSortKey, dir: 'asc' }
  } as SortState,
  filters: {
    userId: '',
    department: '',
...
const pointsRows = computed<PointsRecord[]>(() => sortPointsRows(state.points, state.search.points, state.sort.points))
...
function sortPointsRows(items: PointsRecord[], query: string, sort: SortState['points']) {
  let data = applySearch([...items], query, (item) => item.name || '')
  data.sort((a, b) => {
    switch (sort.key) {
      case 'id':
        return compareValues(a.id, b.id, sort.dir)
      case 'points':
        return compareValues(a.points, b.points, sort.dir)
      case 'name':
      default:
        return compareValues(a.name, b.name, sort.dir)
    }
...
async function loadPoints() {
  state.points = await api.fetchPointsList()
  state.points.forEach((user) => {
    pointsDraft[user.id] = user.points ?? 0
  })
}

async function loadAllData() {
  await Promise.all([loadStats(), loadTodayAttendance(), loadUsers(), loadPoints(), loadAllAttendance()])
}
...
<template>
  ...
  <div class="container">
    ...
    <div class="tabs">
      ...
      <button class="tab" :class="{ active: activeTab === 'points' }" @click="activeTab = 'points'">Punten</button>
    </div>
    ...
    <div id="points" class="tab-content" :class="{ active: activeTab === 'points' }">
      <div class="card">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem;">
          <h2 style="margin:0;">Punten</h2>
          <div style="display:flex;gap:0.5rem;align-items:center;">
            <input v-model="state.search.points" class="table-search" type="text" placeholder="Search name..." />
            <button class="btn btn-secondary" @click="loadPoints">Refresh Points</button>
          </div>
        </div>
        <div v-if="pointsRows.length === 0" class="no-data">No users found</div>
        <table v-else>
          <thead>
            <tr>
              <th class="sortable" data-table="points" @click="updateSort('points', 'id')">ID</th>
              <th class="sortable" data-table="points" @click="updateSort('points', 'name')">Name</th>
              <th class="sortable" data-table="points" @click="updateSort('points', 'points')">Points</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="user in pointsRows" :key="user.id">
              <td>{{ user.id }}</td>
