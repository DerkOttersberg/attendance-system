(() => {
    ...
    function renderDesks() {
        ...
        state.desks.forEach(desk => {
            ...
            deskLabel.textContent = desk.name || 'Desk';
    ...
    function renderDetails() {
        ...
        if (!desk) {
            elements.details.innerHTML = '<div class="floorplan-empty">Select a desk to edit its assignments.</div>';
            return;
        }
        ...
        elements.details.innerHTML = `
            ...
            <div class="slot-actions">
                <button class="btn btn-secondary" id="clearDeskBtn">Clear assignments</button>
                <button class="btn btn-danger" id="deleteDeskBtn">Delete desk</button>
            </div>
        `;
        ...
        if (nameInput) {
            nameInput.addEventListener('input', () => {
                desk.name = nameInput.value.trim() || 'Desk';
                saveState();
                renderDesks();
                renderTable();
            });
    ...
    function renderTable() {
        ...
        if (filteredRows.length === 0) {
            elements.table.innerHTML = '<div class="no-data">No desks available.</div>';
            return;
        }
        ...
        elements.table.innerHTML = `
            <table>
                <thead>
                    <tr>
                        <th class="sortable" data-sort="desk">Desk</th>
    ...
    function clampDeskToRoom(desk) {
        const room = getRoomById(desk.room);
        if (!room) return;
        const minX = room.x + DESK_BORDER;
        const minY = room.y + DESK_BORDER;
        const maxX = room.x + room.w - desk.w - DESK_BORDER;
        const maxY = room.y + room.h - desk.h - DESK_BORDER;
        desk.x = clamp(desk.x, minX, maxX);
        desk.y = clamp(desk.y, minY, maxY);
    }
