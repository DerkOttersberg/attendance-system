...
const Export = {
    ...
    async generatePDFWithLayout(records, selectedUser = null) {
        ...
        // Column positions for 5 columns (datum, aantal, eendheden, product, signature)
        const col1X = 14;      // datum
        const col2X = 50;      // aantal
        const col3X = 85;      // eendheden
        const col4X = 120;     // product
        const col5X = 155;     // signature
        const rowHeight = 6;
        ...
        for (const userName in recordsByUser) {
            ...
            for (let i = 0; i < userRecords.length; i++) {
                ...
                doc.text(product, col4X + 2, yPos + 5);
    ...
    ,
    async generateFloorplanPDF() {
        if (!window.FloorplanAPI || !window.FloorplanAPI.getCanvas) {
            alert('Floorplan is not ready yet.');
            return;
        }

        const canvasEl = window.FloorplanAPI.getCanvas();
        if (!canvasEl) {
            alert('Floorplan is not available.');
            return;
        }

        const dayKey = window.FloorplanAPI.getExportDay();
        const dayLabel = window.FloorplanAPI.getDayLabel(dayKey);
        const legendItems = window.FloorplanAPI.getExportLegend(dayKey);
        ...
        try {
            ...
            console.error('Failed to render floorplan:', err);
            doc.setFontSize(11);
            doc.setFont(undefined, 'normal');
