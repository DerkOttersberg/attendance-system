...
function exportSelectedToPDF() {
    ...
    // Create a new export object with selected records
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();

    const userName = 'Selected Records';
    const dateRange = 'As Selected';
    ...
    doc.text(`Total Records: ${selectedRecords.length}`, 14, 44);
    ...
    const renderRecords = async () => {
        ...
        // Save PDF
        const filename = `Attendance_Selected_${new Date().toISOString().split('T')[0]}.pdf`;
        doc.save(filename);
    };
