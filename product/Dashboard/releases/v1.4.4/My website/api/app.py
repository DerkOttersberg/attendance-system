...
def update_target_website_points(user_name: str, work_duration_minutes: int):
    ...
    try:
        ...
        try:
            ...
            logging.info(f"Found user '{user_name}' with ID {user_id} on target website")
        ...
        if update_response.status_code == 200:
            logging.info(f"✅ Successfully updated {user_name} (ID: {user_id}) on target website with {points} points (worked {work_duration_minutes} minutes)")
            return True
        else:
...
def external_login():
    """Proxy endpoint to login to external API"""
    try:
        # Forward the login request to the external API
        external_url = 'http://10.10.1.6/login/BitsGoes1!'
        
        # Make the request to external API
        response = requests.get(external_url, timeout=5)
        
        logging.info(f"External API login response: {response.status_code}")
        
        if response.status_code == 200:
            return jsonify({
                'success': True,
                'message': 'Successfully logged in to external API'
            }), 200
            ...
            return jsonify({
                'success': False,
                'message': f'Login failed: Server returned status {response.status_code}'
            }), response.status_code
            
    except requests.exceptions.Timeout:
        logging.error("External API login timeout")
        return jsonify({
            'success': False,
            'message': 'Connection timeout: External API not responding'
        }), 500
    except requests.exceptions.ConnectionError as e:
        logging.error(f"External API connection error: {e}")
        return jsonify({
            'success': False,
            'message': f'Connection error: Could not reach external API at 10.10.1.6'
        }), 500
    except Exception as e:
        logging.error(f"External login error: {e}")
        return jsonify({
            'success': False,
...
def list_users():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT id, rfid_uid, name, email, department, product, active FROM users")
