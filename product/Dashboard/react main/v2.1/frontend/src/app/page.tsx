...
export default function Home() {
  ...
  const [addUserModal, setAddUserModal] = useState({ open: false, uid: "", name: "", email: "", password: "", error: "" });
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const { user } = await api.login({ identifier: loginForm.identifier, password: loginForm.password });
      setAuthUser(user);
      setLoginOpen(false);
  ...
  function openAddUserModal() {
    setAddUserModal({ open: true, uid: "", name: "", email: "", password: "", error: "" });
  ...
  async function submitAddUser() {
    ...
    try {
      await api.createUser({
        ...
        name,
        email: addUserModal.email.trim() || null,
  ...
  const loginModal = loginOpen ? (
    <div className="auth-modal">
      <div className="auth-modal-card">
        <h2>Inloggen</h2>
        <p>Inloggen gebeurt op naam (niet hoofdlettergevoelig).</p>
        <form onSubmit={handleLogin}>
          <label>
            Naam (hoofdletters maken niet uit)
            <input
              type="text"
              value={loginForm.identifier}
              onChange={(event) => setLoginForm((prev) => ({ ...prev, identifier: event.target.value }))}
              placeholder="bijv. jan jansen of admin"
              required
            />
          </label>
          <label>
            ...
            <div className="input-with-toggle">
              ...
              >
                {showLoginPassword ? "Verbergen" : "Tonen"}
              </button>
            </div>
          </label>
            ...
            <div className="modal-body">
              ...
              <div className="modal-form">
                ...
                <div className="input-with-toggle">
                  ...
                  >
                    {showAddUserPassword ? "Verbergen" : "Tonen"}
                  </button>
                </div>
