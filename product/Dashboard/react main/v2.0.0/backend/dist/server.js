...
import { all, get, getDbPath, initDb, run } from './db.js';
const app = express();
...
const admin2faRequests = new Map();
...
function createAdmin2faRequest(mode) {
    const requestId = crypto.randomUUID();
    admin2faRequests.set(requestId, { createdAt: Date.now(), mode });
    return requestId;
}
function getAdmin2faRequest(requestId) {
...
function clearAdmin2faRequest(requestId) {
    admin2faRequests.delete(requestId);
}
function getAdmin2faConfig() {
    return get('SELECT secret, enabled FROM admin_2fa WHERE id = 1');
}
function setAdmin2faConfig(secret, enabled) {
    run(`
...
function calculatePoints(workDurationMinutes) {
    if (workDurationMinutes == null)
        return 0;
    return workDurationMinutes < 240 ? 1 : 2;
}
function addPointsForUser(userId, workDurationMinutes) {
...
function logScan(rfidUid, action, success, message) {
    run(`
...
function ensureDepartmentsTable() {
    ...
    `);
    const rows = all('SELECT DISTINCT department FROM users WHERE department IS NOT NULL AND department <> \'\'');
...
function ensureProductsTable() {
    ...
    `);
    const rows = all('SELECT DISTINCT product FROM users WHERE product IS NOT NULL AND product <> \'\'');
    for (const row of rows) {
        if (!row.product)
            continue;
...
function ensureDepartmentId(name) {
    const trimmed = typeof name === 'string' ? name.trim() : '';
    if (!trimmed)
        return null;
    run('INSERT OR IGNORE INTO departments (name) VALUES (?)', [trimmed]);
    const row = get('SELECT id FROM departments WHERE name = ?', [trimmed]);
    return row?.id ?? null;
}
function ensureProductId(name) {
    const trimmed = typeof name === 'string' ? name.trim() : '';
    if (!trimmed)
        return null;
    run('INSERT OR IGNORE INTO products (name) VALUES (?)', [trimmed]);
    const row = get('SELECT id FROM products WHERE name = ?', [trimmed]);
    return row?.id ?? null;
}
...
app.post('/api/auth/login', loginLimiter, async (req, res) => {
    ...
    const user = get('SELECT id, name, email, password_hash FROM users WHERE LOWER(name) = LOWER(?) LIMIT 1', [String(identifier).trim()]);
    ...
    const authUser = { id: user.id, role: 'user', name: user.name, email: user.email };
...
app.post('/api/auth/admin-2fa/verify', (req, res) => {
    ...
    if (!requestId || !code)
        return res.status(400).json({ detail: 'Missing request_id or code' });
    const request = getAdmin2faRequest(String(requestId));
    if (!request)
        return res.status(410).json({ detail: '2FA request expired' });
    const config = getAdmin2faConfig();
    if (!config?.secret)
        return res.status(400).json({ detail: '2FA not configured' });
    ...
    if (!ok)
        return res.status(401).json({ detail: 'Ongeldige 2FA code' });
...
app.get('/api/points/me', (req, res) => {
    if (!req.user?.id)
        return res.status(401).json({ detail: 'Not authenticated' });
    try {
        const row = get('SELECT id, name, points FROM users WHERE id = ?', [req.user.id]);
        if (!row)
...
app.get('/api/debug/last-attendance/:rfidUid', (req, res) => {
    try {
      ...
      `, [rfidUid]);
        if (!row)
            return res.status(404).json({ detail: 'No attendance record found' });
        res.json({
            ...row,
...
app.get('/api/debug/attendance/:rfidUid', (req, res) => {
    try {
        const rfidUid = String(req.params.rfidUid).trim().toUpperCase();
        const user = get('SELECT id, name FROM users WHERE rfid_uid = ?', [rfidUid]);
        if (!user)
            return res.status(404).json({ detail: 'User not found' });
        const rows = all(`
...
app.get('/api/debug/user/:rfidUid', (req, res) => {
    try {
        const rfidUid = String(req.params.rfidUid).trim().toUpperCase();
        const user = get('SELECT id, rfid_uid, name, department, active FROM users WHERE rfid_uid = ?', [rfidUid]);
        if (!user)
            return res.status(404).json({ detail: 'User not found' });
        res.json(user);
    }
...
app.get('/api/user-points/id/:userId', (req, res) => {
    try {
        ...
        const user = get('SELECT id, name, points FROM users WHERE id = ?', [userId]);
        if (!user)
            return res.status(404).json({ detail: 'User not found' });
...
app.post('/api/update-points', (req, res) => {
    try {
        ...
        if (workDuration == null)
            return res.status(400).json({ detail: 'Missing work_duration_minutes' });
        if (!userId && !userName)
            return res.status(400).json({ detail: 'Missing user_id or user_name' });
        const user = get(userId ? 'SELECT id FROM users WHERE id = ?' : 'SELECT id FROM users WHERE name = ?', [userId ?? userName]);
        if (!user)
            return res.status(404).json({ detail: 'User not found' });
...
app.post('/api/scan', (req, res) => {
    ...
    if (!rfidUidRaw)
        return res.status(400).json({ detail: 'Missing rfid_uid' });
    const rfidUid = String(rfidUidRaw).trim().toUpperCase();
    if (debugApi) {
        console.log(`[scan] rfid_uid=${rfidUid} db=${getDbPath()}`);
    }
    try {
        const user = get('SELECT * FROM users WHERE rfid_uid = ? AND active = 1', [rfidUid]);
        if (!user) {
      ...
      WHERE id = ?
      `, [attendance.id]);
        const durationRow = get('SELECT work_duration FROM attendance WHERE id = ?', [
            attendance.id
        ]);
...
app.put('/api/points/:userId', (req, res) => {
    ...
    if (Number.isNaN(points) || points < 0)
        return res.status(400).json({ detail: 'Points must be 0 or higher' });
    try {
...
app.get('/api/floorplan', (_req, res) => {
    try {
        const row = get('SELECT data, updated_at FROM floorplan_layout WHERE id = 1');
        if (!row)
...
app.put('/api/floorplan', (req, res) => {
    const data = req.body;
    if (data == null)
        return res.status(400).json({ detail: 'Missing floorplan data' });
    try {
...
app.post('/api/users', async (req, res) => {
    ...
    try {
        ...
        const result = run('INSERT INTO users (rfid_uid, name, email, department, department_id, password_hash) VALUES (?, ?, ?, ?, ?, ?)', [rfidUid, name, email ?? null, department ?? null, departmentId, passwordHash]);
...
app.delete('/api/users', (req, res) => {
    ...
    if (!Array.isArray(ids) || ids.length === 0)
        return res.status(400).json({ detail: 'ids must be a non-empty list' });
    try {
...
app.post('/api/attendance_last_30', (req, res) => {
    ...
    if (!rfidUid)
        return res.status(400).json({ detail: 'Missing rfid_uid' });
    try {
        const user = get('SELECT id FROM users WHERE rfid_uid = ? AND active = 1', [rfidUid]);
        if (!user)
            return res.status(404).json({ detail: 'User not found' });
        const rows = all(`
...
app.get('/api/attendance/filter', (req, res) => {
    try {
        ...
        const params = [];
        let queryText = 'SELECT a.id, a.date, u.name, u.department, u.product, a.clock_in, a.clock_out, a.work_duration, a.status, a.signature_data ' +
        ...
        const rows = all(queryText, params);
        res.json(rows.map((record) => ({
            ...record,
...
app.post('/api/attendance/manual', (req, res) => {
    try {
        ...
        const user = get('SELECT id, name FROM users WHERE id = ? AND active = 1', [
            Number(user_id)
        ]);
        if (!user)
            return res.status(404).json({ detail: 'User not found' });
...
app.post('/api/attendance/delete', (req, res) => {
    const ids = req.body?.ids;
    if (!Array.isArray(ids) || ids.length === 0)
        return res.status(400).json({ detail: 'No records to delete' });
    try {
        const placeholders = ids.map(() => '?').join(',');
...
app.get('/api/departments', (_req, res) => {
    try {
        ...
        const rows = all('SELECT id, name FROM departments ORDER BY name ASC');
        res.json(rows);
    }
    catch (error) {
        res.status(500).json({ detail: 'Database connection failed' });
...
app.post('/api/departments', (req, res) => {
    ...
    if (!name)
        return res.status(400).json({ detail: 'Missing department name' });
    try {
        ensureDepartmentsTable();
...
app.delete('/api/departments/:deptId', (req, res) => {
    ...
    try {
        ensureDepartmentsTable();
        const row = get('SELECT name FROM departments WHERE id = ?', [deptId]);
        if (!row)
...
app.get('/api/products', (_req, res) => {
    try {
        ensureProductsTable();
        const rows = all('SELECT id, name FROM products ORDER BY name ASC');
        res.json(rows);
    }
    catch (error) {
        res.status(500).json({ detail: 'Database connection failed' });
...
app.post('/api/products', (req, res) => {
    const name = String(req.body?.name ?? '').trim();
    if (!name)
        return res.status(400).json({ detail: 'Missing product name' });
    try {
        ensureProductsTable();
...
app.delete('/api/products/:productId', (req, res) => {
    ...
    try {
        ensureProductsTable();
        const row = get('SELECT name FROM products WHERE id = ?', [productId]);
        if (!row)
...
app.put('/api/users/:userId/uid', (req, res) => {
    ...
    if (!newUid)
        return res.status(400).json({ detail: 'Missing rfid_uid' });
    try {
        const dup = get('SELECT id FROM users WHERE rfid_uid = ? AND id != ?', [
            newUid,
            Number(req.params.userId)
        ]);
        if (dup)
            return res.status(400).json({ detail: 'RFID UID already in use' });
...
app.put('/api/users/:userId', (req, res) => {
    ...
    const updates = {};
    const idUpdates = {};
    for (const key of ['name', 'email', 'department', 'rfid_uid']) {
    ...
    if (updates.rfid_uid)
        updates.rfid_uid = updates.rfid_uid.toUpperCase();
    const setParts = [
