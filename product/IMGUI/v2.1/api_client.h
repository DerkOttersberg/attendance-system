...
#define API_BASE_URL "https://10.10.0.3"
#define API_ATTENDANCE_PATH "/api/attendance_last_30"


//de genereerde certs van het dashboard in de dashbaord directory /infra/mlts/..... zitten de certs
#define API_CA_CERT_PATH "/etc/ssl/certs/attendance/ca.crt"
#define API_CLIENT_CERT_PATH "/etc/ssl/certs/attendance/client.crt"
#define API_CLIENT_KEY_PATH "/etc/ssl/certs/attendance/client.key"
...
private:
    ...
    void ConfigureTls(CURL* curl) {
        curl_easy_setopt(curl, CURLOPT_CAINFO, API_CA_CERT_PATH);
        curl_easy_setopt(curl, CURLOPT_SSLCERT, API_CLIENT_CERT_PATH);
        curl_easy_setopt(curl, CURLOPT_SSLKEY, API_CLIENT_KEY_PATH);
...
public:
    ...
    bool FetchAttendanceLast30Days(const std::string& raw_rfid_uid, std::vector<std::string>& out_dates) {
        ...
        char url[256];
        snprintf(url, sizeof(url), "%s%s", API_BASE_URL, API_ATTENDANCE_PATH);

        std::string json = "{\"rfid_uid\":\"" + cleaned_rfid_uid + "\"}";
