...
class SignatureScreen(FloatLayout):
    """Screen for capturing signatures"""
    ...
    def __init__(self, on_submit, on_timeout, **kwargs):
        ...
        instruction_label = Label(
            text='Please sign below to clock in',
            font_size='24sp',
            size_hint=(1, 0.08),
            pos_hint={'x': 0, 'y': 0.77},
            color=(0.3, 0.3, 0.3, 1)
        ...
        # Signature drawing area (white background)
        signature_container = FloatLayout(
            size_hint=(0.9, 0.5),
            pos_hint={'center_x': 0.5, 'y': 0.2}
        )
        
        with signature_container.canvas.before:
            Color(1, 1, 1, 1)
            self.sig_bg = Rectangle(pos=signature_container.pos, size=signature_container.size)
        
        signature_container.bind(pos=self._update_sig_bg, size=self._update_sig_bg)
        
        self.signature_widget = SignatureWidget(size_hint=(1, 1))
        signature_container.add_widget(self.signature_widget)
        self.add_widget(signature_container)
        
        # Timeout label
        self.timeout_label = Label(
            text=f'Time remaining: {SIGNATURE_TIMEOUT}s',
            font_size='18sp',
            size_hint=(1, 0.06),
            pos_hint={'x': 0, 'y': 0.13},
            color=(0.5, 0.5, 0.5, 1)
    ...
    def _update_sig_bg(self, instance, value):
        self.sig_bg.pos = instance.pos
        self.sig_bg.size = instance.size
    
    def start_timeout(self):
        """Start the timeout countdown"""
        self.time_remaining = SIGNATURE_TIMEOUT
        self.timeout_event = Clock.schedule_interval(self._update_timeout, 1)
    
    def _update_timeout(self, dt):
        """Update timeout counter"""
        self.time_remaining -= 1
        self.timeout_label.text = f'Time remaining: {self.time_remaining}s'
