...
void Buzzer_On(void);
void Buzzer_PlayTone(uint16_t period, uint16_t pulse, uint32_t duration_ms);
void Buzzer_SetPeriodAndPulse(uint16_t period, uint16_t pulse);
void red_led_off();
void red_led_on();
void green_led_off();
void green_led_on();
...
int main(void)
{
  ...
  // Initialize MFRC522
   ...
   qprint("  buzz        - Test buzzer\r\n");
   qprint("  beep        - Test single beep\r\n");
   qprint("  red_on         - Turn red LED on\r\n");
   qprint("  red_off        - Turn red LED off\r\n");
   qprint("  green_on       - Turn green LED on\r\n");
   qprint("  green_off      - Turn green LED off\r\n");
   qprint("===================\r\n\r\n");
...
void ProcessCommand(char* cmd)
{
    ...
    } else if (strncmp(cmd, "buzz", 4) == 0) {
        qprint(">> Buzzing...\r\n");
        Buzzer_PlayTone(500, 100, 50);
        HAL_Delay(50);
        Buzzer_PlayTone(500, 100, 50);
        HAL_Delay(50);
        Buzzer_PlayTone(500, 100, 50);
    ...
    } else if (strncmp(cmd, "red_on", 6) == 0) {
        qprint(">> Red LED ON\r\n");
        red_led_on();

    } else if (strncmp(cmd, "red_off", 7) == 0) {
        qprint(">> Red LED OFF\r\n");
        red_led_off();

    } else if (strncmp(cmd, "green_on", 8) == 0) {
        qprint(">> Green LED ON\r\n");
        green_led_on();

    } else if (strncmp(cmd, "green_off", 9) == 0) {
        qprint(">> Green LED OFF\r\n");
        green_led_off();
...
void Buzzer_SetPeriodAndPulse(uint16_t period, uint16_t pulse)
{
    // Stop PWM first for safe register modification (optional but safer)
    HAL_TIM_PWM_Stop(&htim16, TIM_CHANNEL_1);

    // Set the new Period (ARR register) for pitch
    __HAL_TIM_SET_AUTORELOAD(&htim16, period);

    // Set the new Pulse (CCR register) for duty cycle/volume
    __HAL_TIM_SET_COMPARE(&htim16, TIM_CHANNEL_1, pulse);

    // Re-start the PWM with new settings
    HAL_TIM_PWM_Start(&htim16, TIM_CHANNEL_1);
}

/**
 * @brief Plays a specific tone (pitch and volume) for a duration
 */
void Buzzer_PlayTone(uint16_t period, uint16_t pulse, uint32_t duration_ms)
{
    Buzzer_SetPeriodAndPulse(period, pulse);
