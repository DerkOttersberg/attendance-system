...
void ProcessTouchInput(ImGuiIO& io, std::vector<ImVec2>& drawPoints, bool& isDrawing)
{
    ...
    static ImVec2 last_touch_pos = ImVec2(0, 0);  // ✅ Remember last position
    ...
    if (is_touching)
    {
        ...
        last_touch_pos = ImVec2(draw_x, draw_y);  // ✅ Save position

        if (updated)
        {
            drawPoints.push_back(ImVec2(draw_x, draw_y));
            isDrawing = true;
    ...
    // ✅ Always set position (use last known position if currently touching)
    ...
    if (was_touching && !is_touching)
    {
        ...
        printf("🔵 Touch released at (%.0f, %.0f)\n", last_touch_pos.x, last_touch_pos.y);
    }
